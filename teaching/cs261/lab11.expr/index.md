...
- Now, move on to the `buildTree()` method that you'll need to implement. First, you'll need a stack that's capable of storing *both* kinds of nodes. From the given postfix expression, start scanning each term from left to right. When you encounter a constant value in the given postfix expression, make a new `ValueNode` with that `double` value stored inside of it, and push it onto the stack. When an operator (like *, -, /, and +) is encountered, create a new `BinOpNode` and store the operator inside. Moreover, you need to pop two nodes off the stack and assign them to be the left and right child of this node, forming a new tree. Then push this new tree onto the stack. 

- There should be exactly one node left on the stack, which is reference to the full expression tree! So just assign the `root` instance variable of this class to point to whatever's on the top of the stack! If there are too many or too few nodes on the stack at this point, then the expression is incorrect or incomplete. Feel free to throw a RuntimeException if that's the case.
