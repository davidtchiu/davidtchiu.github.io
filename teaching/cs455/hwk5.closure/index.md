...
3. Now open the `FDUtil` class, and implement the following static methods:
    ...
    - `augment(FDSet fdset, Set<String> attrs)` -- This method returns a new set of FDs generated by combining both sides of each FD  with the given set of attributes `attrs`. Here's an example output for $$FD = \{A \rightarrow B, AB \rightarrow C\}$$ augmented with attribute set $$\{C\}$$:
