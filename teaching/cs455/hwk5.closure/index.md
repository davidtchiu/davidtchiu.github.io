...
3. Now open the `FDUtil` class, and implement the following static methods:
  ...
  - `public static FDSet trivial(FDSet fdset)` -- This method accepts a set of FDs and returns a set of trivial functional dependencies. Recall the trivial rule specifies that any subset of the left-side attributes can be (trivially) determined by the left-side attributes.
  ...
  - `public static FDSet augment(FDSet fdset, Set<String> attrs)` -- This method returns a new set of FDs generated by combining of both sides of each FD  with the given set of attributes `attrs`.
  ...
  - `public static FDSet transitive(FDSet fdset)` -- This method returns a new set of FDs after repeatedly applying the transitive rule until no more changes are detected. 
  ...
  - Finally, `public static FDSet fdSetClosure(FDSet fdset)` -- This method accepts a set of FDs and returns its closure, i.e., the full set of FDs generated through the repeated applications of Armstrong's Axioms. 
